...
router.post("/", async (req, res) => {
  try {
    ...
    return res.status(500).json({
      error:
        "Sorry, we were unable to create a new category at this time. Please try again later.",
    });
  }
});
  ...
  try {
    ...
    if (!category_name) {
      return res.status(400).json({
        error: "Values undefined",
        message:
          "Please provide the new category name and id of the category you'd like to update.",
      });
    }
